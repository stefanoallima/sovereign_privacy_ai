...
function splitForCanvas(content: string): { intro: string; canvas: string } {
  ...
  const intro =
    docStart > 0 ? paragraphs.slice(0, docStart).join("\n\n").trim() : "";
...
function shouldAutoCanvas(content: string): boolean {
  ...
  const hasEmojiStep =
    /^[\p{Emoji_Presentation}\p{Extended_Pictographic}].*(step|tip|note|\d+)/imu.test(
      content
    );
  return (
    wordCount > 100 ||
...
export function ChatWindow() {
  ...
  const { createDocument, openPanel, getDocumentsByConversation } =
    useCanvasStore();
  ...
  const persona = useMemo(
    () => personas.find((p) => p.id === conversationPersonaId),
    [personas, conversationPersonaId]
  );
  const project = useMemo(
    () => projects.find((p) => p.id === conversationProjectId),
  ...
  const activePrivacyMode = useMemo(
    () => getActivePrivacyMode(persona),
    ...
    const specialMentions = [
      {
        ...
        name: "here",
        icon: "📍",
        description: "All active personas in this thread",
        isSpecial: true,
      },
      {
        id: "_all",
    ...
    (e: React.ChangeEvent<HTMLTextAreaElement>) => {
      ...
      if (lastAtRateIndex !== -1) {
        const isStartOrSpace =
          lastAtRateIndex === 0 ||
          textBeforeCursor[lastAtRateIndex - 1] === " ";
  ...
  const selectMention = (selectedItem: any) => {
    ...
    const mentionText = selectedItem.isSpecial
      ? selectedItem.name
      : selectedItem.name;
  ...
  useEffect(() => {
    ...
    if (!isLoading && msgs.length >= 2 && conv?.title === "New Conversation") {
      ...
      generateTitleFor(currentConversationId, userMsg, assistantMsg).finally(
        () => {
  ...
  useEffect(() => {
    const retitleStale = async () => {
      ...
      const stale = allConvs.filter(
        (c) => c.title === "New Conversation" && !c.isIncognito
      );
  ...
  const handleSend = async () => {
    ...
    if (pendingAttachment?.isFormFill) {
      ...
      const userMsgs =
        msgs[currentConversationId]?.filter((m) => m.role === "user") || [];
...
${attachment.textContent}`;
    ...
    if (targetPersonas.length > 1) {
      ...
      await sendMessage(
        message,
        targetPersonas.length === 1 ? targetPersonas : undefined,
        attachmentForMessage
      );
    }
  };
  ...
  const handlePrivacyModeSelect = (mode: "local" | "hybrid" | "cloud") => {
    ...
    if (currentConversationId) {
      ...
      const modelId =
        mode === "local"
          ? (ollamaModels.find((m) => m.apiModelId === s.localModeModel)?.id ??
            ollamaModels[0]?.id)
          : mode === "hybrid"
            ? s.hybridModeModel
            : s.cloudModeModel;
  ...
  const parseAndAttach = useCallback(async (filePath: string) => {
    try {
      ...
      const fileType =
        parsed.file_type.toLowerCase() as FileAttachment["fileType"];
  ...
  const handleAttachmentSendAsContext = useCallback(() => {
    ...
    setPendingAttachment((prev) =>
      prev ? { ...prev, isFormFill: false } : null
    );
  }, [pendingAttachment]);
  ...
  const handleAttachmentFillForm = useCallback(() => {
    if (!pendingAttachment) return;
    setPendingAttachment((prev) =>
      prev ? { ...prev, isFormFill: true } : null
    );
  }, [pendingAttachment]);
    ...
    return (
      <div className="flex flex-1 flex-col items-center justify-center text-center p-8">
        ...
        <div className="mb-10 animate-emblem-enter">
          <div className="relative mx-auto h-24 w-24">
            ...
            <div className="absolute inset-1 rounded-xl bg-[hsl(var(--primary))] flex items-center justify-center shadow-lg shadow-[hsl(var(--primary)/0.3)]">
              <Lock
                className="h-10 w-10 text-[hsl(var(--primary-foreground))]"
                strokeWidth={1.8}
              />
            </div>
            {/* Gold corner accent */}
        ...
        <p className="max-w-sm text-[hsl(var(--muted-foreground))] mb-10 text-base leading-relaxed animate-fade-in">
          Private coaching, therapy, and brainstorming — nothing leaves your
        ...
        {!hasApiKey && !isAirplane && (
          <div className="rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--surface-2))] px-4 py-3 max-w-md animate-slide-up flex items-center gap-3">
            ...
            <p className="text-sm text-[hsl(var(--foreground-muted))]">
              ...
              in Settings to enable cloud models.
            </p>
          </div>
        )}
        ...
        {modelStatus && !modelStatus.is_downloaded && (
          <div className="rounded-2xl border border-[hsl(var(--border)/0.5)] bg-[hsl(var(--card)/0.8)] backdrop-blur-sm p-6 max-w-md mb-6 animate-slide-up">
            ...
            {isDownloadingModel ? (
              ...
              >
                Download Privacy Engine (~
                {(modelStatus.model_size_bytes / (1024 * 1024 * 1024)).toFixed(
                  1
                )}{" "}
                GB)
              </button>
        ...
        <div className="grid grid-cols-2 gap-3 max-w-lg w-full stagger-children">
          {personas.slice(0, 4).map((p) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold group-hover:text-[hsl(var(--primary))]">
                  {p.name}
                </p>
                <p className="text-xs text-[hsl(var(--muted-foreground))] line-clamp-2 mt-0.5 leading-relaxed">
                  {p.description}
                </p>
              </div>
            </button>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold group-hover:text-[hsl(var(--violet))]">
                Incognito Chat
              </p>
              <p className="text-xs text-[hsl(var(--muted-foreground))] mt-0.5">
                Messages vanish when you close or leave.
              </p>
            </div>
          </button>
        </div>
  ...
  return (
    <div
      className={`flex flex-1 flex-col overflow-hidden bg-[hsl(var(--background))] relative ${
        activePrivacyMode === "local"
          ? "privacy-tint-local"
          : activePrivacyMode === "hybrid"
            ? "privacy-tint-hybrid"
            : activePrivacyMode === "cloud"
              ? "privacy-tint-cloud"
              : ""
      }`}
    >
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-[hsl(var(--border))] bg-[hsl(var(--surface-1))] flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            {persona && <span className="text-lg">{persona.icon}</span>}
            ...
            <div className="space-y-6 px-4">
                ...
                return (
                  <div key={message.id}>
                    <MessageBubble
                      ...
                      onViewCanvas={
                        canvasDocId ? () => openPanel(canvasDocId) : undefined
                      }
                      onOpenCanvas={
                        canvasDocId
                          ? undefined
              ...
              {streamingContent && (
                <div className="animate-fade-in">
                  <MessageBubble
                    role="assistant"
                    content={streamingContent.replace(
                      /<think>[\s\S]*?<\/think>/gi,
                      ""
                    )}
                    isStreaming
              ...
              {isLoading && !streamingContent && (
                <div className="flex gap-4 px-4 py-3 animate-fade-in">
                  ...
                  <div className="flex items-center gap-1.5 pt-3">
                    <div
                      className="h-2 w-2 bg-[hsl(var(--primary))] rounded-full animate-bounce"
              ...
              {/* Gap Fill Prompt */}
              {formFill.currentStep === "gap-filling" &&
                formFill.gapFields.length > 0 &&
                formFill.currentGapIndex < formFill.gapFields.length && (
                  <div className="animate-fade-in">
                    <GapFillPrompt
                      key={
                        formFill.gapFields[formFill.currentGapIndex]?.id ||
                        formFill.currentGapIndex
                      }
                      ...
                      onSubmit={(value, saveToProfile) => {
                        ...
                        const { currentGapIndex, gapFields } =
                          useFormFillStore.getState();
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-[hsl(var(--background))] via-[hsl(var(--background)/0.95)] to-transparent pointer-events-none">
        <div className="mx-auto max-w-3xl w-full pointer-events-auto">
          ...
          {/* Floating Input Box */}
          <div
            className={`relative border border-[hsl(var(--border))] bg-[hsl(var(--surface-2))] rounded-2xl focus-within:border-[hsl(var(--ring)/0.4)] focus-within:ring-2 focus-within:ring-[hsl(var(--ring)/0.08)] focus-within:shadow-[var(--shadow-md)] shadow-[var(--shadow)] ${pendingReview ? "opacity-40 pointer-events-none" : ""}`}
          >
                ...
                <div className="p-1 max-h-64 overflow-y-auto">
                  {filteredPersonas.map((p: any, i) => (
                    ...
                    >
                      ...
                      {i === mentionIndex && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-[hsl(var(--secondary))] opacity-70">
                          ↵
                        </span>
                      )}
                    </button>
              ...
              {[
                {
                  ...
                  label: "Local",
                  activeCls:
                    "border-green-600/40 bg-green-500/12 text-green-700 dark:text-green-400 dark:border-green-500/40 privacy-pill-active",
                  modelLabel: (() => {
                ...
                },
                {
                  ...
                  label: "Hybrid",
                  activeCls:
                    "border-[hsl(var(--primary)/0.4)] bg-[hsl(var(--primary)/0.1)] text-[hsl(var(--primary))] privacy-pill-active",
                  modelLabel: (() => {
                    const m = useSettingsStore
                      .getState()
                ...
                },
                {
                  ...
                  label: "Cloud",
                  activeCls:
                    "border-amber-500/40 bg-amber-500/10 text-amber-700 dark:text-amber-400 dark:border-amber-500/40 privacy-pill-active",
                  modelLabel: (() => {
                    const m = useSettingsStore
                      .getState()
                ...
                return (
                  <button
                    ...
                    title={
                      mode === "local"
                        ? "All processing on your device — maximum privacy"
                        : mode === "hybrid"
                          ? "PII redacted locally, then sent to cloud LLM"
                          : "Direct to cloud API — fastest"
                    }
                  >
                    <Icon className="h-3.5 w-3.5" />
                    <span className="leading-none">{label}</span>
                    <span className="text-[10px] opacity-50 leading-none font-medium">
                      {modelLabel}
                    </span>
                  </button>
                );
              ...
              <div className="flex items-center justify-between px-4 pb-4">
                <div className="flex items-center gap-2">
                  ...
                  {privacyStatus.mode === "idle" && (
                    <div
                      className="hidden sm:flex items-center gap-1.5 flex-wrap"
                      data-tour="privacy-badge"
                    >
                      ...
                      <span className="text-[10px] text-[hsl(var(--muted-foreground)/0.3)]">
                  ...
                  {privacyStatus.mode !== "idle" && (
                    <span
                      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium transition-all ${
                        privacyStatus.mode === "processing"
                          ? "bg-[hsl(var(--primary)/0.1)] text-[hsl(var(--primary))] animate-pulse"
                          : privacyStatus.mode === "pending_review"
                            ? "bg-amber-500/10 text-amber-400 animate-pulse"
                            : privacyStatus.mode === "local"
                              ? "bg-green-500/10 text-green-400"
                              : privacyStatus.mode === "attributes_only"
                                ? "bg-green-500/10 text-green-400"
                                : privacyStatus.mode === "anonymized"
                                  ? "bg-[hsl(var(--primary)/0.1)] text-[hsl(var(--primary))]"
                                  : privacyStatus.mode === "blocked"
          ...
          <p className="mt-4 text-center text-[11px] text-[hsl(var(--muted-foreground)/0.5)]">
            {activePrivacyMode === "local"
              ...
              : activePrivacyMode === "hybrid"
                ? "🛡️ Hybrid — PII redacted locally, then cloud LLM"
                : activePrivacyMode === "custom"
                  ? "⚙️ Custom — persona-specific routing"
                  : "⚡ Cloud — direct to Nebius"}
            {customTermsCount > 0 && (
              <span className="text-pink-500">
                {" "}
                · {customTermsCount} custom redaction
                {customTermsCount !== 1 ? "s" : ""} active
              </span>
            )}
