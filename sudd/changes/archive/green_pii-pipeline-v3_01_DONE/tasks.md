...
- [x] Confirm no real callers: `grep -rn "sendPrivacyAwareChat\|streamPrivacyAwareChat" apps/desktop/src`
- [x] Delete the two functions + dead helpers/imports
...
- [x] Refactor GLiNER-redaction core to be callable on arbitrary text
- [x] Call GLiNER + term-matching inside maybeRedact (GLiNER first)
- [x] Merge GLiNER mappings into allMappings
- [x] Auto-persist detected entities (dedup-guarded)
- [x] Fallback to term-matching only if GLiNER unavailable
- [x] Manual check: history-only PII becomes a placeholder in the assembled payload
...
- [x] Verify presence of redact_messages_command (redaction.rs / redaction_commands.rs / lib.rs); implement TDD if missing
- [x] `cargo test redaction` passes (incl. batch test)
- [x] Wire executePrivacySend history loop to redact_messages_command (preserve roles)
